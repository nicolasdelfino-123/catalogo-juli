...
export default function InicioNuevo() {
    ...
    const marqueeConfig = storeConfig.homeMarquee || {};
    const marqueeTexts = Array.isArray(marqueeConfig.texts)
        ? marqueeConfig.texts.filter(Boolean)
        : [];
    const showHomeMarquee = marqueeConfig.enabled !== false && marqueeTexts.length > 0;
    const marqueeSeparator = marqueeConfig.separator || "•";
                        ...
                        {[0, 1].map((groupIndex) => (
                            ...
                            >
                                {[0, 1, 2].map((repeatIndex) => (
                                    <React.Fragment key={`${groupIndex}-${repeatIndex}`}>
                                        {marqueeTexts.map((text, textIndex) => (
                                            <React.Fragment key={`${groupIndex}-${repeatIndex}-${text}-${textIndex}`}>
...
"
            ...
            {/* Bloque de contacto y mapa desactivado temporalmente. */}
            {false && (
